...
import com.spdx.hms.v1.mapper.ICollegeStudentContactMapper;
...
@RestController
@Slf4j
@RequestMapping(path = "/api/hms/colleges/studentcontact", consumes = APPLICATION_API_SPIDEX_V1_JSON,
        produces = APPLICATION_API_SPIDEX_V1_JSON)
public class CollegeStudentContactController {
	
	@Autowired
    private ICollegeStudentContactMapper mapper;
    @Autowired
    private ICollegeStudentContactService service;
    
    @PostMapping
    public CollegeStudentContactResponse create(@Valid @RequestBody CollegeStudentContactSaveRequest request) {
        try {
    ...
    @PutMapping
    public CollegeStudentContactResponse update(@RequestBody CollegeStudentContactUpdateRequest request) {
        try {
            return Optional.ofNullable(request)
                    .map(mapper::map)
                    .map(service::update)
                    .map(mapper::map)
                    .orElseGet(CollegeStudentContactResponse::new);
        } catch (HMSException hmsEx) {
            throw hmsEx;
        } catch (Exception ex) {
    ...
    @GetMapping(path = "/{id}")
    public CollegeStudentContactResponse retrieve(@PathVariable Long id) {
        try {
            ...
            log.error("Error occurred while retrieving CollegeStudentContact", ex);
            throw new HMSException(ErrorCode.HMS0001, ErrorCode.HMS0001.getMessage(), ex.getMessage(), ex);
        }
    }
    
    @GetMapping()
    public List<CollegeStudentContactResponse> retrieve() {
        try {
    ...
    private CollegeStudentContactGetRequestDto map(Long id) {
        return CollegeStudentContactGetRequestDto.builder().contactId(id).build();
    }
