...
import com.spdx.hms.v1.mapper.IStudentReportingInfoMapper;
...
@RestController
@Slf4j
@RequestMapping(path = "/api/hms/studentsreportinginfo/", consumes = APPLICATION_API_SPIDEX_V1_JSON,
...
public class StudentReportingInfoController {
	
		@Autowired
	    private IStudentReportingInfoMapper mapper;
	    @Autowired
	    private IStudentReportingInfoService service;
	
	    
	    @PostMapping
	    public StudentReportingInfoResponse create(@Valid @RequestBody StudentReportingInfoSaveRequest request) {
	        try {
	            ...
	            log.error("Error occurred while saving studentsReportingInfo", ex);
	   ...
	   @PutMapping
	    public StudentReportingInfoResponse update(@RequestBody StudentReportingInfoUpdateRequest request) {
	        try {
	            return Optional.ofNullable(request)
	                    .map(mapper::map)
	                    .map(service::update)
	            ...
	            log.error("Error occurred while updating studentReportingInfo", ex);
	            throw new HMSException(ErrorCode.HMS0001, ErrorCode.HMS0001.getMessage(), ex.getMessage(), ex);
	        }
	    }
	    ...
	    public ResponseEntity<String> delete(@PathVariable String id) {
	        try {
	            ...
	            log.error("Error occurred while deleting studentReporintInfo", ex);
	            throw new HMSException(ErrorCode.HMS0023, ErrorCode.HMS0023.getMessage(), ex.getMessage(), ex);
	        }
	    }
	   
	   
	    @GetMapping(path = "{id}")
	    public StudentReportingInfoResponse retrieve(@PathVariable String id) {
	       	 try {
	             return Optional.ofNullable(id)
	                     .map(this::map)
	             ...
	             log.error("Error occurred while retrieving studentReporintInfo", ex);
	             throw new HMSException(ErrorCode.HMS0024, ErrorCode.HMS0024.getMessage(), ex.getMessage(), ex);
	         }
	     }
	
	    @GetMapping()
	    public List<StudentReportingInfoResponse> retrieve() {
	        try {
	            ...
	            log.error("Error occurred while retrieving StudentReportinInfo", ex);
	            throw new HMSException(ErrorCode.HMS0023, ErrorCode.HMS0023.getMessage(), ex.getMessage(), ex);
	        }
	    }

	    private StudentReportingInfoGetRequestDto map(String id) {
	        return StudentReportingInfoGetRequestDto.builder()
	                .stdAdmissionId(Long.valueOf(id))
	                .build();
	    }
